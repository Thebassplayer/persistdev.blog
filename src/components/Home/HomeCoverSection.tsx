...
const HomeCoverSection = ({ posts }: HomeCoverSectionProps) => {
  ...
  const linkTag = firstPostTag ? `/categories/${slug(firstPostTag)}` : "#";
  const coverImage = latestPost?.image?.filePath.replace("../public", "") ?? "";
  const bluredImage = latestPost?.image?.blurhashDataUrl;
